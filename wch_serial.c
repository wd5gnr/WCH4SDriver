  ...
  )
{
  ...
  drv->state = kzalloc(sizeof(struct ser_state) * drv->nr, GFP_KERNEL);
  ...
  // init tty_port structures
  for (i=0;i<drv->nr;i++) 
    {
      normal->ports[i]=kmalloc(sizeof(struct tty_port), GFP_KERNEL);
      if (!normal->ports[i]) 
	{
	  printk("WCH Error: Allocate tty_port fail!\n\n");
	  goto out;
	}
      tty_port_init(normal->ports[i]);
    }
      
    

  drv->tty_driver = normal;
  // Note: some of what needs to be set is already set in alloc
  normal->name = drv->dev_name;
  normal->driver_name="wch";
